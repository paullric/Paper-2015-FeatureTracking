...
As observed by \cite{neu2013imilast} and others, feature tracking schemes can produce wildly varying results depending on the specific choice of threshold variables and values.  Consequently, we argue that conclusions drawn from these tracking schemes should use an ensemble of detection schemes.
...
Once the complete set of tropical cyclone paths has been computed, total tropical cyclone density over each 0.5 degree grid cell is plotted in Figure \ref{fig:TropicalCycloneDensity}.  Overall the results show very good agreement with reference fields ({\color{red}Citation?}).
...
{\small \begin{verbatim}
  ./DetectCyclonesUnstructured    --in_data "$uvfile;$hfile" --out $outf    --searchbymax "ABS_V_L100(1)" --mergedist 2.0    --closedcontourcmd "ABS_V_L100(1),-5.e-5,4,0"    --outputcmd "ABS_V_L100(1),max,0;HGT_L1,max,0"    --maxlat 30.0\end{verbatim}}

{\small \begin{verbatim}
  ./StitchNodes    --in pgbhnl.dcu_aew_all.dat --out pgbhnl.dcu_aew_stitch.dat    --format "i,j,lon,lat,absv,zs?    --range 4.0 --minlength 8 --maxgap 0    --min_endpoint_dist 16.0    --threshold "lat,>=,0,8"
\end{verbatim}}
...
\end{center}
\caption{Tropical cyclone trajectories and associated intensities as obtained from the simulation of a single hurricane season in CESM \ref{sec:TropicalCycloneCESM} using (top) native ne120 spectral-element grid data and (bottom) data regridded to a regular latitude-longitude grid with 0.25$^\circ$ grid spacing.} \label{fig:TropicalCycloneCESM}
\end{figure}
